"use client";

import React from "react";
import { motion } from "framer-motion";
import Link from "next/link";
import { 
  ScanLine, 
  TrendingUp, 
  Target, 
  FolderKanban, 
  FileSpreadsheet,
  ArrowUpRight
} from "lucide-react";
import { cn } from "@/shared/lib/utils";

// Fitur utama FINUSA dengan deskripsi yang mudah dipahami
const finusaFeatures = [
  {
    id: "receipt-scanner",
    title: "Scan Struk AI",
    tag: "OCR AI",
    desc: "Pindai struk belanja otomatis catat pengeluaran",
    icon: <ScanLine className="w-5 h-5 text-blue-400" aria-hidden="true" />,
    badgeClass: "text-blue-400 bg-blue-500/10 border-blue-500/25",
    iconBg: "bg-blue-500/15 border-blue-500/30",
    href: "/receipt-scanner",
  },
  {
    id: "monitor",
    title: "Monitor Keuangan",
    tag: "Analisis",
    desc: "Pantau arus kas, grafik, dan saldo rekening",
    icon: <TrendingUp className="w-5 h-5 text-emerald-400" aria-hidden="true" />,
    badgeClass: "text-emerald-400 bg-emerald-500/10 border-emerald-500/25",
    iconBg: "bg-emerald-500/15 border-emerald-500/30",
    href: "/monitor",
  },
  {
    id: "nabung",
    title: "Target Tabungan",
    tag: "Impian",
    desc: "Kelola pos tabungan dan celengan impian",
    icon: <Target className="w-5 h-5 text-teal-400" aria-hidden="true" />,
    badgeClass: "text-teal-400 bg-teal-500/10 border-teal-500/25",
    iconBg: "bg-teal-500/15 border-teal-500/30",
    href: "/nabung",
  },
  {
    id: "catat",
    title: "Catat Transaksi",
    tag: "Manual",
    desc: "Catat uang masuk dan keluar secara cepat",
    icon: <FolderKanban className="w-5 h-5 text-amber-400" aria-hidden="true" />,
    badgeClass: "text-amber-400 bg-amber-500/10 border-amber-500/25",
    iconBg: "bg-amber-500/15 border-amber-500/30",
    href: "/catat",
  },
  {
    id: "sheets",
    title: "Template Sheets",
    tag: "Spreadsheet",
    desc: "Export dan rekap data di Google Sheets",
    icon: <FileSpreadsheet className="w-5 h-5 text-sky-400" aria-hidden="true" />,
    badgeClass: "text-sky-400 bg-sky-500/10 border-sky-500/25",
    iconBg: "bg-sky-500/15 border-sky-500/30",
    href: "/template",
  },
];

export default function QuickAccess() {
  const renderCardContent = (feat: typeof finusaFeatures[0]) => (
    <div className="h-full rounded-2xl border border-slate-800 bg-[#161c28] p-4 transition-all duration-200 hover:border-blue-500/40 hover:bg-[#1a2130] flex flex-col justify-between shadow-md">
      <div>
        {/* Top Row: Icon + Badge + Arrow */}
        <div className="flex items-center justify-between gap-2 mb-3">
          <div className={`w-9 h-9 sm:w-10 sm:h-10 rounded-xl flex items-center justify-center border ${feat.iconBg}`}>
            {feat.icon}
          </div>
          <div className="flex items-center gap-1.5">
            <span className={`text-[10px] font-bold px-2 py-0.5 rounded-md border uppercase tracking-wider ${feat.badgeClass}`}>
              {feat.tag}
            </span>
            <ArrowUpRight
              className="w-4 h-4 text-slate-500 transition-all duration-200 group-hover:text-white group-hover:translate-x-0.5 group-hover:-translate-y-0.5"
              aria-hidden="true"
            />
          </div>
        </div>

        {/* Title */}
        <h3 className="text-sm font-bold text-white group-hover:text-blue-300 transition-colors">
          {feat.title}
        </h3>

        {/* Description */}
        <p className="text-xs text-slate-400 mt-1.5 leading-relaxed line-clamp-2">
          {feat.desc}
        </p>
      </div>

      {/* Bottom Action Footer */}
      <div className="mt-4 pt-2.5 border-t border-slate-800/80 flex items-center justify-between text-xs font-medium text-slate-400 group-hover:text-blue-400">
        <span>Buka Modul</span>
        <span className="font-bold">→</span>
      </div>
    </div>
  );

  return (
    <div className="space-y-3.5">
      {/* Section Header */}
      <div className="flex items-center justify-between px-1">
        <div>
          <h2 className="text-base sm:text-lg font-bold text-white flex items-center gap-2">
            <span className="inline-block h-2 w-2 rounded-full bg-blue-500" aria-hidden="true" />
            Akses Cepat Fitur FINUSA
          </h2>
          <p className="text-xs text-slate-400 mt-0.5">Modul utama pengelolaan finansial pelajar & umum</p>
        </div>
      </div>

      {/* Mobile view: Horizontal Scroll Snap Layout (1st card largest on left, rest follow to the right) */}
      <div className="flex sm:hidden overflow-x-auto snap-x snap-mandatory gap-3 pb-3 pt-1 -mx-4 px-4 [&::-webkit-scrollbar]:hidden">
        {finusaFeatures.map((feat, i) => (
          <Link
            href={feat.href}
            key={feat.id}
            className={cn(
              "group block snap-start shrink-0 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-blue-500",
              i === 0 ? "w-[82%]" : "w-[72%]"
            )}
          >
            <motion.div
              initial={{ opacity: 0, x: 20 }}
              animate={{ opacity: 1, x: 0 }}
              transition={{ duration: 0.3, delay: i * 0.05 }}
              className="h-full"
            >
              {renderCardContent(feat)}
            </motion.div>
          </Link>
        ))}
      </div>

      {/* Desktop view: Standard 5-column grid */}
      <div className="hidden sm:grid sm:grid-cols-2 lg:grid-cols-5 gap-3.5">
        {finusaFeatures.map((feat, i) => (
          <Link
            href={feat.href}
            key={feat.id}
            className="group block rounded-xl focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-blue-500"
          >
            <motion.div
              initial={{ opacity: 0, y: 12 }}
              animate={{ opacity: 1, y: 0 }}
              transition={{ duration: 0.35, delay: i * 0.05 }}
              className="h-full"
            >
              {renderCardContent(feat)}
            </motion.div>
          </Link>
        ))}
      </div>
    </div>
  );
}
